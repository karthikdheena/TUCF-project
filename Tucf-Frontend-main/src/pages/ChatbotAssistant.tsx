  ...
  interface Window {
    pdfjsLib?: {
      ...
      getDocument: (options: { data: ArrayBuffer }) => {
        promise: Promise<ChatbotPdfDocument>;
      };
    };
  }
}
...
const CHATBOT_MAX_HISTORY_MESSAGES = 6;
const CHATBOT_MAX_MESSAGE_CHARS = 1200;
const CHATBOT_MAX_RESUME_CONTEXT_CHARS = 3500;
const CHATBOT_TEXT_MAX_TOKENS = 900;
const CHATBOT_VISION_MAX_TOKENS = 1200;
...
function chatbot_renderMarkdown(text: string) {
  ...
  return escaped
    .replace(
      /```[\w]*\n?([\s\S]*?)```/g,
      '<pre style="background:#0d1117;padding:12px;border-radius:8px;overflow-x:auto;font-size:12px;border:1px solid #2a2a2a"><code>$1</code></pre>',
    )
    ...
    .replace(
      /^\d+\. (.*$)/gm,
      '<li style="margin:4px 0;list-style:decimal inside">$1</li>',
    )
    .replace(
      /^[-•] (.*$)/gm,
      '<li style="margin:4px 0;list-style:disc inside">$1</li>',
    )
    .replace(/\n\n/g, "<br><br>")
    .replace(/\n/g, "<br>");
}

function chatbot_truncateText(text: string, maxChars: number) {
...
function chatbot_compactHistory(history: ChatbotApiMessage[]) {
  const recent = history.slice(-CHATBOT_MAX_HISTORY_MESSAGES);
  ...
  return recent
    .map((message) => {
      ...
      return {
        role: message.role,
        content: chatbot_truncateText(
          message.content,
          CHATBOT_MAX_MESSAGE_CHARS,
        ),
      } as ChatbotApiMessage;
    })
    .filter((message): message is ChatbotApiMessage => Boolean(message));
}
...
async function chatbot_ensurePdfJs() {
  ...
  await new Promise<void>((resolve, reject) => {
    const existing = document.querySelector(
      'script[data-chatbot-pdfjs="true"]',
  ...
  const pdfjsLib = window.pdfjsLib as NonNullable<Window["pdfjsLib"]>;
...
async function chatbot_streamGroq(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const rawMessage =
        errorPayload?.error?.message || `Groq API error ${response.status}`;
      const friendlyMessage =
        typeof rawMessage === "string" &&
        /tokens per minute|TPM|Request too large/i.test(rawMessage)
          ? "Request is too large for current Groq TPM limits. Try a shorter message or fewer attached details."
          : rawMessage;
...
const ChatbotAssistant: React.FC = () => {
  ...
  const [chatbot_currentFile, setChatbot_currentFile] = useState<File | null>(
    null,
  );
  const [chatbot_currentFilePreview, setChatbot_currentFilePreview] =
    useState<string>("");
  ...
  const chatbot_scrollBottom = () => {
    if (chatbot_messagesRef.current) {
      chatbot_messagesRef.current.scrollTop =
        chatbot_messagesRef.current.scrollHeight;
    }
  };
  ...
  useEffect(() => {
    ...
    chatbot_textareaRef.current.style.overflowY =
      chatbot_textareaRef.current.scrollHeight > maxHeight ? "auto" : "hidden";
  }, [chatbot_input]);
  ...
  const chatbot_appendUserBubble = (
    text: string,
    file: File | null,
    previewUrl?: string,
  ) => {
  ...
  const chatbot_updateBubble = (
    messageId: string,
    content: string,
    role: "assistant" | "error" = "assistant",
  ) => {
    setChatbot_messages((current) =>
      current.map((message) =>
  ...
  const chatbot_handleFileChange = async (
  ...
  const chatbot_send = async () => {
    ...
    const compactResumeText = effectiveResumeText
      ? chatbot_truncateText(
          effectiveResumeText,
          CHATBOT_MAX_RESUME_CONTEXT_CHARS,
        )
      : "";
...
For all other questions (not resume related), answer helpfully and concisely.${compactResumeText ? `\nUser's Resume Content:\n${compactResumeText}` : ""}`;

    const compactHistory = chatbot_compactHistory(chatbot_history);
    const messages: ChatbotApiMessage[] = [
      { role: "system", content: systemPrompt },
      ...compactHistory,
    ];

    let chatbot_userContent = chatbot_truncateText(
      userText,
      CHATBOT_MAX_MESSAGE_CHARS,
    );
    if (fileContext) {
      chatbot_userContent = `${chatbot_truncateText(userText || "Please analyze my resume", CHATBOT_MAX_MESSAGE_CHARS)}\n\nResume:\n${chatbot_truncateText(fileContext, CHATBOT_MAX_RESUME_CONTEXT_CHARS)}`;
    }
    ...
    if (isImage) {
      ...
      const textMessage: ChatbotApiMessage = {
        role: "user",
        content: chatbot_userContent,
      };
    ...
    const maxTokens = isImage
      ? CHATBOT_VISION_MAX_TOKENS
      : CHATBOT_TEXT_MAX_TOKENS;
  ...
  return (
    <div className="chatbot-page">
      <div className="chatbot-shell">
        ...
        {!chatbot_hasApiKey && (
          <div className="chatbot-banner">Add Groq API key in Settings</div>
        )}

        <div className="chatbot-messages" ref={chatbot_messagesRef}>
          {chatbot_messages.map((message) => {
            const chatbot_hasLatex =
              message.content.includes("\\documentclass[letterpaper") ||
              message.content.includes("```latex");
            const chatbot_latex = chatbot_hasLatex
              ? chatbot_extractLatex(message.content)
              : "";
                  ...
                  {message.file && (
                    <div className="chatbot-file-preview">
                      {message.file.type === "application/pdf" ? (
                        <div className="chatbot-pdf-chip">
                          PDF {message.file.name}
                        </div>
                      ) : message.file.previewUrl ? (
                        <img
                          src={message.file.previewUrl}
                          alt={message.file.name}
                          className="chatbot-image-thumb"
                        />
                      ) : null}
                    </div>
                  )}
                      ...
                      {chatbot_text && (
                        <div
                          ...
                          dangerouslySetInnerHTML={{
                            __html: chatbot_renderMarkdown(chatbot_text),
                          }}
                        />
                      )}
                        ...
                        <div className="chatbot-latex-wrap">
                          <div className="chatbot-latex-actions">
                            <button
                              type="button"
                              onClick={() =>
                                void chatbot_handleCopyLatex(message.content)
                              }
                            >
                              Copy LaTeX
                            </button>
                            <button
                              type="button"
                              onClick={() =>
                                chatbot_handleDownloadLatex(message.content)
                              }
                            >
                              Download .tex
                            </button>
                          </div>
                          <pre className="chatbot-latex-code">
                            <code>{chatbot_latex}</code>
                          </pre>
                        </div>
                      )}
                    </>
        ...
        <div className="chatbot-inputbar">
          ...
          <div className="chatbot-inputrow">
            ...
            <button
              ...
              disabled={
                chatbot_streaming ||
                (!chatbot_input.trim() && !chatbot_currentFile)
              }
            >
